...
$.extend(SVGWrapper.prototype, {
	...
	loadMapToElement: function(parent ,url, settings) {
		settings = (typeof settings == 'boolean'? {addTo: settings} :
			(typeof settings == 'function'? {onLoad: settings} : settings || {}));
		if (!settings.addTo) {
			this.clear(false);
		...
		var loadSVG = function(data) {
			...
			wrapper.change(parent, attrs);
			var nodes = data.documentElement.childNodes;
			for (var i = 0; i < nodes.length; i++) {
				try {
					if ($.svg._renesis) { //判断是否是IE
						throw 'Force traversal';
					}
					//firefox下添加子节点
	...
	_toSVG: function(node) {
		...
		if (node.nodeType == 3) { // Text
			...
			if (node.attributes) {
				//对node.attributes.length进行了缓存，提高js速度
				for (var i = 0, attrlength = node.attributes.length; i < attrlength; i++) {
					var attr = node.attributes.item(i);
...
$.extend(SVGPath.prototype, {
	...
	/* Draw a vertical line to a position.f
	...
	/* Continue a quadratic B閦ier curve.
...
$.fn.svg = function(options) {
	...
	if (typeof options == 'string' && options == 'get') {
		// alert("otherArgs:"+otherArgs.toString());
